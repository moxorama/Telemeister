...
async function createHandler(
  ...
  console.log(`📝 Created: src/handlers/${stateName}/index.ts`);
}
...
`;
    ...
    console.log(`📝 Created: src/handlers/index.ts`);
    return;
  }
...
function removeFromHandlersIndex(stateName: string, cwd: string): void {
  ...
  if (content.includes(importLine)) {
    ...
    fs.writeFileSync(indexPath, content);
    console.log(`📝 Removed import from: src/handlers/index.ts`);
  }
}
...
export async function stateAdd(stateName: string | undefined): Promise<void> {
  ...
  if (!validateStateName(stateName)) {
    console.error(
      '❌ Error: State name must start with a letter and contain only letters, numbers, and underscores'
    );
    process.exit(1);
  }
  ...
  console.log(`📝 Updated: src/bot-state-types.ts`);
...
export async function stateDelete(stateName: string | undefined): Promise<void> {
  ...
  if (fs.existsSync(handlerDir)) {
    ...
    console.log(`🗑️  Removed empty folder: src/handlers/${stateName}/`);
  }
...
export async function stateSync(): Promise<void> {
  ...
  console.log('🔄 Syncing state types and handlers...\n');
